...
public:
    CudnnLayerNorm(int N, int M, const float* input, float* output, const float* gamma, const float* beta, float epsilon) : handle_(nullptr), x_desc_(nullptr), y_desc_(nullptr), scale_desc_(nullptr), bias_desc_(nullptr), epsilon_desc_(nullptr), mean_desc_(nullptr), inv_variance_desc_(nullptr), operation_(nullptr), operation_graph_(nullptr), engine_config_(nullptr), execution_plan_(nullptr), variant_pack_(nullptr), workspace_(nullptr), d_mean_(nullptr), d_inv_variance_(nullptr), workspace_size_(0), epsilon_(epsilon) {
        ...
        mean_desc_ = create_tensor_descriptor(5, statistic_dimensions, statistic_strides, 4, false);
        inv_variance_desc_ = create_tensor_descriptor(6, statistic_dimensions, statistic_strides, 4, false);
        ...
        CUDNN_CHECK(cudnnBackendSetAttribute(operation_, CUDNN_ATTR_OPERATION_NORM_FWD_MEAN_DESC, CUDNN_TYPE_BACKEND_DESCRIPTOR, 1, &mean_desc_));
        ...
        CUDA_CHECK(cudaMalloc(&d_mean_, (size_t)N * sizeof(float)));
        CUDA_CHECK(cudaMalloc(&d_inv_variance_, (size_t)N * sizeof(float)));
        create_variant_pack(input, output, gamma, beta);
    }
    ...
    ~CudnnLayerNorm() {
        ...
        destroy_backend_descriptor(mean_desc_);
        ...
        if (d_mean_ != nullptr) cudaFree(d_mean_);
...
private:
    void create_execution_plan() {
        ...
        if (execution_plan_ == nullptr) {
            fprintf(stderr, "cuDNN did not return a usable LayerNorm execution plan\n");
            exit(EXIT_FAILURE);
        }
    ...
    void create_variant_pack(const float* input, float* output, const float* gamma, const float* beta) {
        int64_t uids[7] = {0, 1, 2, 3, 4, 5, 6};
        void* data_pointers[7] = {(void*)input, (void*)output, (void*)gamma, (void*)beta, (void*)&epsilon_, d_mean_, d_inv_variance_};
        CUDNN_CHECK(cudnnBackendCreateDescriptor(CUDNN_BACKEND_VARIANT_PACK_DESCRIPTOR, &variant_pack_));
        CUDNN_CHECK(cudnnBackendSetAttribute(variant_pack_, CUDNN_ATTR_VARIANT_PACK_UNIQUE_IDS, CUDNN_TYPE_INT64, 7, uids));
        CUDNN_CHECK(cudnnBackendSetAttribute(variant_pack_, CUDNN_ATTR_VARIANT_PACK_DATA_POINTERS, CUDNN_TYPE_VOID_PTR, 7, data_pointers));
    ...
    cudnnBackendDescriptor_t mean_desc_;
...
__global__ void layernorm_shared(const float *input, float *output, int N, int M, const float* gamma, const float* beta, float epsilon) {
    ...
    if (warp_idx == 0) {
        thread_sum = 0;
        if (lane_idx < blockDim.x / WARP_SIZE) thread_sum = shared_warp[lane_idx];
        for (int offset = WARP_SIZE / 2; offset > 0; offset >>= 1) {
            thread_sum += __shfl_down_sync(0xffffffff, thread_sum, offset);
        }
        if (lane_idx == 0) shared_warp[0] = thread_sum / static_cast<float>(M);
    }
    __syncthreads();
    float thread_mean = shared_warp[0];
    __syncthreads();

    // apply mean & sum_squares
    float thread_sum_squares = 0;
    for (int i = local_thread_idx; i < M; i+=blockDim.x) {
        float val = shared_row[i];
        val -= thread_mean;
    ...
    for (int i = local_thread_idx; i < M; i += blockDim.x) {
        ...
        output_row[i] = centered * inv_std * gamma[i] + beta[i];
    }
}

template <int BSIZE>
void layernorm_shared_launcher(const float *input, float *output, int N, int M, const float* gamma, const float* beta, float epsilon) {
    if (M > 4096) return;
    dim3 grid_size(N);
    dim3 block_size(BSIZE);
    int num_warps = BSIZE / WARP_SIZE;
    size_t shared_bytes = (static_cast<size_t>(M) + num_warps) * sizeof(float);
    layernorm_shared<<<grid_size, block_size, shared_bytes>>>(input, output, N, M, gamma, beta, epsilon);
}

template<int BLOCK_SIZE, int BUCKET_SIZE>
__global__ void layernorm_register(const float *input, float *output, int N, int M, const float* gamma, const float* beta, float epsilon) {
    int row_idx = blockIdx.x;
    if (row_idx >= N) return;
    ...
    if (warp_idx == 0) {
        thread_sum = 0;
        if (lane_idx < NUM_WARPS) thread_sum = shared_warp[lane_idx];
        for (int offset = WARP_SIZE / 2; offset > 0; offset >>= 1) {
            thread_sum += __shfl_down_sync(0xffffffff, thread_sum, offset);
        }
        if (lane_idx == 0) shared_warp[0] = thread_sum / static_cast<float>(M);
    ...
    for (int reg_idx = 0; reg_idx < REG_SIZE; reg_idx++) {
        int col_idx = BLOCK_SIZE  * reg_idx + local_thread_idx;
        if (col_idx < M) {
            reg_input[reg_idx] -= thread_mean;
            thread_sum_squares += reg_input[reg_idx] * reg_input[reg_idx];
        }
    }
    // warp_sum_squares 内规约
    for (int offset = WARP_SIZE / 2; offset > 0; offset >>= 1) {
    ...
    for (int reg_idx = 0; reg_idx < REG_SIZE; reg_idx++) {
        int col_idx = BLOCK_SIZE  * reg_idx + local_thread_idx;
        if (col_idx < M) {
            output_row[col_idx] = reg_input[reg_idx] * inv_std * gamma[col_idx] + beta[col_idx];
        }
    }
}

template <int BSIZE>
void layernorm_register_launcher(const float *input, float *output, int N, int M, const float* gamma, const float* beta, float epsilon) {
    if (M > 4096) return;
    dim3 grid_size(N);
    dim3 block_size(BSIZE);
    if (M <= 128) {
        layernorm_register<BSIZE, 128><<<grid_size, block_size>>>(input, output, N, M, gamma, beta, epsilon);
    }
    else if (M <= 256) {
        layernorm_register<BSIZE, 256><<<grid_size, block_size>>>(input, output, N, M, gamma, beta, epsilon);
    }
    else if (M <= 512) {
        layernorm_register<BSIZE, 512><<<grid_size, block_size>>>(input, output, N, M, gamma, beta, epsilon);
    }
    else if (M <= 1024) {
        layernorm_register<BSIZE, 1024><<<grid_size, block_size>>>(input, output, N, M, gamma, beta, epsilon);
    }
    else if (M <= 2048) {
        layernorm_register<BSIZE, 2048><<<grid_size, block_size>>>(input, output, N, M, gamma, beta, epsilon);
    }
    else if (M <= 4096) {
        layernorm_register<BSIZE, 4096><<<grid_size, block_size>>>(input, output, N, M, gamma, beta, epsilon);
    }
    else {
        return;
    }
...
struct TestContext {
    ...
    TestContext(int N_value, int M_value) : N(N_value), M(M_value), element_count((size_t)N_value * (size_t)M_value), tensor_bytes(element_count * sizeof(float)), parameter_bytes((size_t)M_value * sizeof(float)), epsilon(1e-5f), input((float*)malloc(tensor_bytes)), output((float*)malloc(tensor_bytes)), gamma((float*)malloc(parameter_bytes)), beta((float*)malloc(parameter_bytes)), d_input(nullptr), d_output(nullptr), d_gamma(nullptr), d_beta(nullptr) {
        if (input == nullptr || output == nullptr || gamma == nullptr || beta == nullptr) {
            fprintf(stderr, "Host memory allocation failed\n");
            exit(EXIT_FAILURE);
        }
        for (size_t i = 0; i < element_count; ++i) input[i] = (float)((int)((i * 37u + 17u) % 1001u) - 500) / 100.0f;
        for (int i = 0; i < M; ++i) {
            gamma[i] = 0.5f + (float)((i * 13) % 101) / 100.0f;
            beta[i] = (float)(((i * 7) % 101) - 50) / 100.0f;
        }
        ...
        CUDA_CHECK(cudaMemcpy(d_beta, beta, parameter_bytes, cudaMemcpyHostToDevice));
    }
...
double* layernorm_cpu_reference(const TestContext& context) {
    ...
    for (int row = 0; row < context.N; ++row) {
        ...
        double mean = 0.0;
        for (int col = 0; col < context.M; ++col) mean += (double)context.input[row_offset + col];
        mean /= (double)context.M;
        double variance = 0.0;
        for (int col = 0; col < context.M; ++col) {
            double centered = (double)context.input[row_offset + col] - mean;
            variance += centered * centered;
        }
        variance /= (double)context.M;
        double inv_std = 1.0 / sqrt(variance + (double)context.epsilon);
        for (int col = 0; col < context.M; ++col) reference[row_offset + col] = ((double)context.input[row_offset + col] - mean) * inv_std * (double)context.gamma[col] + (double)context.beta[col];
    }
    return reference;
}
...
int main(int argc, char* argv[]) {
    ...
    CudnnLayerNorm cudnn(options.N, options.M, context.d_input, context.d_output, context.d_gamma, context.d_beta, context.epsilon);
    double* reference = nullptr;
    if (options.mode == RUN_VERIFY) {
        reference = layernorm_cpu_reference(context);
        printf("%-20s%-8sErrors\n", "Kernel", "Status");
    ...
    run_candidate("shared_128", custom_supported, options, context, reference_ptr, &matched, &succeeded, [&] { layernorm_shared_launcher<128>(context.d_input, context.d_output, context.N, context.M, context.d_gamma, context.d_beta, context.epsilon); });
    run_candidate("shared_256", custom_supported, options, context, reference_ptr, &matched, &succeeded, [&] { layernorm_shared_launcher<256>(context.d_input, context.d_output, context.N, context.M, context.d_gamma, context.d_beta, context.epsilon); });
    run_candidate("shared_512", custom_supported, options, context, reference_ptr, &matched, &succeeded, [&] { layernorm_shared_launcher<512>(context.d_input, context.d_output, context.N, context.M, context.d_gamma, context.d_beta, context.epsilon); });
    run_candidate("register_128", custom_supported, options, context, reference_ptr, &matched, &succeeded, [&] { layernorm_register_launcher<128>(context.d_input, context.d_output, context.N, context.M, context.d_gamma, context.d_beta, context.epsilon); });
    run_candidate("register_256", custom_supported, options, context, reference_ptr, &matched, &succeeded, [&] { layernorm_register_launcher<256>(context.d_input, context.d_output, context.N, context.M, context.d_gamma, context.d_beta, context.epsilon); });
    run_candidate("register_512", custom_supported, options, context, reference_ptr, &matched, &succeeded, [&] { layernorm_register_launcher<512>(context.d_input, context.d_output, context.N, context.M, context.d_gamma, context.d_beta, context.epsilon); });
    if (!matched) {
